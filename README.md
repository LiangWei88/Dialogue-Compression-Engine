...
| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `Padding` | `0.3s` | 对白前后额外保留的缓冲时间，防止剪辑生硬。 |
| `合并阈值` | `0.5s` | 两个片段间隔小于此值时自动合并，避免画面频繁闪跳。 |
| `最小长度` | `0.5s` | 过滤掉时长过短的杂音或无效字幕。 |
| `目标大小` | `None` | 输入期望的 MB 数，脚本会自动反推最佳比特率。 |
| `无损模式` | `False` | 开启后不重编码（速度极快），但对 WebM 等格式兼容性较差。 |
